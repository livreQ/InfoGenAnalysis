...
class BaseVAE(BaseModel):
    ...
    def decode(self, z: torch.Tensor) -> torch.Tensor:
        ...
        return (self._decoder(z), self._decoder2(z))
    ...
    def forward(
        ...
        x_hat, x_hat_prime = self.decode(z)
        # print(x_hat.type, x_hat_prime.type)
        return  x_hat, x_hat_prime, mu, logvar
    ...
    def loss_function(
        ...
        true: torch.Tensor,
        pred: torch.Tensor,
        pred_prime: torch.Tensor,
        mu: torch.Tensor,
        logvar: torch.Tensor,
    ) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        logpxz = self.log_pxz(pred, true)
        logpxz_prime = self.log_pxz(pred_prime, true)
        KLD = -0.5 * torch.sum(1 + logvar - mu.pow(2) - logvar.exp())
        REC = -logpxz.sum()
        MI = logpxz.sum() - logpxz_prime.sum()
        return (REC + KLD, KLD, REC, MI)
    ...
    def step(self, batch: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        x_pred, x_pred_prime,  mu, logvar = self(batch)
        ##x_pred mu_{\theta}(z), mu_{\phi}(z), \sigma^2_{\phi}(z)
        loss, KLD, REC, MI = self.loss_function(batch, x_pred, x_pred_prime, mu, logvar)
        return (loss, KLD, REC, MI)
...
class BernoulliMixin:
    def log_pxz(self, pred: torch.Tensor, true: torch.Tensor) -> torch.Tensor:
        ...
        try:
            ...
            BCE = -binary_cross_entropy(pred[0], true, reduction="none")   
...
class BernoulliMLPVAE(BernoulliMixin, BaseVAE):
    ...
    def construct_model(self):
        ...
        for l in reversed(self._layers):
            ...
            decoder2.append(nn.Linear(prev_dim, l))
            decoder.append(nn.ReLU(True))
            decoder2.append(nn.ReLU(True))
            prev_dim = l
        decoder.append(nn.Linear(prev_dim, input_shape))
        decoder.append(nn.Sigmoid())
        decoder.append(nn.Unflatten(1, (C, D, D)))
        decoder2.append(nn.Linear(prev_dim, input_shape))
        decoder2.append(nn.Sigmoid())
        decoder2.append(nn.Unflatten(1, (C, D, D)))
        self._decoder = nn.Sequential(*decoder)
        self._decoder2 = nn.Sequential(*decoder2)
