...
class BaseTrainer(metaclass=abc.ABCMeta):
    ...
    def train(self, model: BaseModel, loader: DataLoader, epoch: int):
        ...
        for b, (batch_x, _, batch_idxs) in enumerate(loader):
            ...
            loss, KLD, REC, MI = model.step(batch_x)
            loss.backward()
        ...
        avg_REC = total_REC /len(loader.dataset)
        avg_MI = total_MI / len(loader.dataset)
        print(f"=====> Epoch: {epoch}. Average train loss: {avg_loss:.4f}")
        print(f"=====> Epoch: {epoch}. Average KLD: {avg_KLD:.4f}")
        print(f"=====> Epoch: {epoch}. Average REC: {avg_REC:.4f}")
        print(f"=====> Epoch: {epoch}. Average MI: {avg_MI:.4f}")
        return (avg_loss, avg_KLD, avg_REC, avg_MI)
...
class MarginalTrainer(BaseTrainer):
    def fit(
        ...
        for epoch in range(1, self._epochs + 1):
            t0 = time.time()
            # (avg_loss, avg_KLD, avg_REC, avg_MI)
            loss_train, KLD_train, REC_train, MI_train = self.train(model, train_loader, epoch)
            if test_loader:
                loss_test, KLD_test, REC_test, MI_test  = self.test(model, test_loader, epoch)
            t1 = time.time()
            ...
            losses["train"].append((loss_train, KLD_train, REC_train, MI_train))
            if test_loader:
                losses["test"].append((loss_test, KLD_test, REC_test, MI_test))
